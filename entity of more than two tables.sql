...
select count(sub_id) from submissions;



select q.q_id ,count(*),q.q_name,q.rating
from submissions s
join question q
on s.question_id = q.q_id
where status = 1 
group by q.q_name,q.q_id,q.rating
order by q.q_id;


select distinct q.contest_id,count(*)
from
 submissions s join question q
on  q.q_id = s.question_id
group by q.contest_id;

select distinct q.contest_id,q.q_id,q.q_name,count(*)
from
 submissions s join question q
on  q.q_id = s.question_id
...
------------------------------------------
select c.c_name,count(*),c.c_date
from
 submissions s join question q 
on  q.q_id = s.question_id
join contest c
...
-----------------------------------------------------
select  u.name,c.c_name,count(*),q.q_name as total_submission
from
 users u
 join
 submissions s
...
on  q.q_id = s.question_id
 join contest c
on q.contest_id = c.c_id
group by c.c_name,u.name,q.q_name
order by c.c_name;
