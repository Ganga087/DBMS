...
update employee set department = 'SWE'
where department = 'SW';

update employee set age = 21 
where age is null;

update employee set email = 'hr_dummy@example.com'
where  department = 'HR'  and email is null;
select * from employee;
...
-- DISTINCT -- ON -- SINGLE COLUMN -- MULTIPLE COLUMN
...
order by age desc
nulls last;
-- ORDERBY (ON MULTIPLE COLUMNS)
SELECT ID,NAME,SALARY,AGE,DEPARTMENT,GENDER
FROM EMPLOYEE ORDER BY salary desc nulls last , age , department desc;

-- aggrigate functions
--sum() working on column
-- count() give how many ,and it doesnt count null to get nulls put count(*)
-- avg() ,min(),max()
select count(*) from employee where department = 'Operations' and gender = 'Female'; --12
select count(*) from employee  where email is NULL; -- 31
select count(*) from employee where department = 'Testing' and date_of_joining >= to_date('12-07-2019','DD-MM-YYYY');
----
select count(gender) ,count(age) from employee;
select avg(salary),min(salary),max(salary) from employee where gender = 'Female';
